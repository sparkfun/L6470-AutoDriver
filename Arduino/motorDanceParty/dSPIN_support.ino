...
unsigned long dSPIN_Param(unsigned long value, byte bit_len, byte chip)
{
  ...
  if (byte_len == 3) {
    ret_val |= dSPIN_Xfer((byte)(value>>16), chip) << 16;
    //Serial.println(ret_val, HEX);
  }
  if (byte_len >= 2) {
    ret_val |= dSPIN_Xfer((byte)(value>>8), chip) << 8;
    //Serial.println(ret_val, HEX);
  }
  if (byte_len >= 1) {
...
byte dSPIN_Xfer(byte data, byte chip)
{
  byte data_out;
  switch(chip)
  {
